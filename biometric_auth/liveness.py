...
def extract_face_landmarks(image_data):
    ...
    h, w = rgb.shape[:2]
    # MediaPipe FaceMesh is not safe to call concurrently from overlapping Flask requests.
    # Guard it so frame timestamps stay strictly ordered and do not crash with
    # "Packet timestamp mismatch" during the liveness stream.
    with _MESH_LOCK:
